...
class TestMapper extends WordSpec with MustMatchers {
  ...
  "The insert method" when invoked{
    ...
    "with an object with a key not auto-generated" must{
      "return a new version, that may be the same instance, of the object" in {
        pending
      }
      "insert the object in database" in {
	      pending
    ...
    "inside a mapper that does not define the pk" must {
      "throw an exception" in {
        pending //Normalement impossible
      }
    }
  }
  ...
  "The update method" when invoked{
    ...
    "inside a mapper that does not define the pk" must {
      "throw an exception" in {
        pending // Normalement, pas de sens: pk doit être implémenté
      }
    }
  }
}