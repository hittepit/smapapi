...
trait Mapper[E, I] { this: JdbcTransaction =>
  object PrimaryKey{
    def apply[P](cdef:ColumnDefinition[E,P]) = new PrimaryKeyColumnDefinition(cdef.name,cdef.sqlType,cdef.getter)
  }
  ...
  implicit def columnToColumnDef[P](c:Column[P]):ColumnDef[E,P] = c match{
    case Column(name:String,sqlType) => new ColumnDef(name,c.sqlType)
    case _ => throw new Exception("Conversion impossible") //TODO
  }
  
  def column[E,P](n: String, st: SqlType[P], g: E => P) = new ColumnDefinition[E,P](n,st,g)

  def generatedPrimaryKey[E,I](n: String, st: SqlType[I], g: E => I, s: (E,I)=>E):PrimaryKeyColumnDefinition[E,I] = new AutoGeneratedColumn[E,I](n,st,g,s)
  ...
  def insert(entity: E): E = inTransaction { session =>
    val params = insertable.map{c =>
      Param(c.getter(entity),c.sqlType.asInstanceOf[SqlType[Any]])
    }
    ...
    pk match {
      case key:AutoGeneratedColumn[E,I] =>
        ...
        session.insert(insertSqlString, params,Column(key.name,key.sqlType)) match {
  ...
  def update(entity:E):Unit = inTransaction{session =>
    ...
    val params = updatable.map{c =>
      Param(c.getter(entity),c.sqlType.asInstanceOf[SqlType[Any]])
    }:::List(Param(pk.value(entity),pk.sqlType))
  ...
  def find(id: I): Option[E] = readOnly { session =>
    ...
    val params = List(Param(id,pk.sqlType))
